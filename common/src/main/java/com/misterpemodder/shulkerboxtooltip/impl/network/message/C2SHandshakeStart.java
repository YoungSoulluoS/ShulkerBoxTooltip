  ...
  public static class Type implements MessageType<C2SHandshakeStart> {
    ...
    public void onReceive(C2SHandshakeStart message, MessageContext<C2SHandshakeStart> context) {
      ...
      if (message.clientVersion == null) {
        ShulkerBoxTooltip.LOGGER.error(player.getScoreboardName() + ": received invalid handshake packet");
        channel.unregisterFor(player);
        return;
      }

      // compatibility check
      ShulkerBoxTooltip.LOGGER.info(player.getScoreboardName() + ": protocol version: " + message.clientVersion);
      if (message.clientVersion.major() != ProtocolVersion.CURRENT.major()) {
        ShulkerBoxTooltip.LOGGER.error(player.getScoreboardName() + ": incompatible client protocol version, expected "
